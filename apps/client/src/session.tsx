...
export function useSession() { const value = useContext(Context); if (!value)
    throw new Error('SessionProvider missing'); return value; }
export function useClientData() {
    const { source } = useSession();
    return useQuery({ queryKey: ['workspace', 'lina-fixture-v1'], queryFn: ({ signal }) => source.read(signal) });
}
export function useAcknowledge() {
    const { source } = useSession();
    const cache = useQueryClient();
    return useMutation({ mutationFn: source.acknowledge, onSuccess: () => cache.invalidateQueries({ queryKey: ['workspace', 'lina-fixture-v1'] }) });
}
